// src/controllers/concessions.controller.ts — Capa HTTP
import { Request, Response, NextFunction } from 'express';
import { z } from 'zod';
import * as service from '../services/concessions.service';
import { createConcessionSchema, updateConcessionSchema } from '../schemas/concessions.schema';

const idSchema = z.string().uuid({ error: 'El id debe ser un UUID válido' });
...
export async function getAll(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const result = await service.listConcessions(page, limit);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
export async function getById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const item = await service.getConcession(parsedId.data);
    res.json({ data: item });
  } catch (err) {
    next(err);
  }
